 ...
 * It will be discarded at the end of the batch (defined below).
 *
 ...
 * the work needs to be flushed due to resource constraints, etc). From the implementor's POV,
 * the end is when any staged work is forwarded, after which it is safe for the CDK to acknowledge
 * to the Airbyte platform that the records have been handled. (Specifically, even after a sync
 * failure these records might not be replayed, though some may be.) If records are being pushed
 ...
 * [src/main/resources/application-override.yaml].
 *
 ...
 * the next batch if more data arrives. (Note: close should only be used to do cleanup that must happen even
 * if the sync fails; it should not be used to forward work.)
 *
 ...
 * Specifically, up to `input-parts` DirectLoaders will be created per stream, and each will handle
 * a specific subset of records concurrently, and each subset will be processed in order of receipt.
 *
...
interface DirectLoader : AutoCloseable {
    ...
    /**
     * Called once per record until it returns [Complete], after which [close] is called, the
     * loader is discarded, and the records are considered processed by the
     * platform.
     */
    fun accept(record: DestinationRecordAirbyteValue): DirectLoadResult

    /**
     * Called by the CDK to force work to finish. It will only be called
     * if the last call to [accept] did not return [Complete]. After which
     * [close] is called, the loader is discarded, and the records are considered processed by
     * the platform.
     */
    fun finish()
}
...
abstract class DirectLoaderFactory<T : DirectLoader> {
    abstract fun create(streamDescriptor: DestinationStream.Descriptor, part: Int): T
}
